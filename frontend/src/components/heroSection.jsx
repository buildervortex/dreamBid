            ...
            >
                ...
                <div className="relative z-10 flex flex-col justify-center h-full px-10 md:px-20 lg:px-32 text-white">
                    ...
                    <div className="mt-10 flex space-x-4 items-center carousel-container">
                        {visibleImages.map((image, index) => {
                            return (
                                <div
                                    key={index}
                                    className={`w-32 h-24 bg-gray-700 rounded-md flex items-center justify-center border border-transparent transition duration-300 ease-in-out ${selectedImageIndex === (index === 0 ? selectedImageIndex : (selectedImageIndex + index) % carouselImages.length) ? 'opacity-100' : 'opacity-50 hover:opacity-70'
                                        }`}
                                    onClick={() => handleImageClick(image, (selectedImageIndex + index) % carouselImages.length)}
                                >
                                    <img
                                        src={image}
                    ...
                    <div className="absolute bottom-0 right-0 left-0 flex justify-center mb-10">
                        ...
                        >
                            <span className="text-2xl font-semibold rotate-180">&#8593;</span> {/* Up arrow symbol */}
                        </button>
                    </div>
                </div>
            </section>
